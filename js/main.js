$(document).ready(function() {

    var bgArray = ['2.jpg', '3.jpg', '4.jpg', '5.jpg', '6.jpg', '7.jpg', '8.jpg', '9.jpg', '10.jpg', '11.jpg', '12.jpg', '13.jpg', '14.jpg', '15.jpg', '16.jpg', '17.jpg', '18.jpg', '19.jpg', '20.jpg', '21.jpg', '22.jpg', '23.jpg', '24.jpg', '25.jpg', '26.jpg', '27.jpg', '28.jpg', '29.jpg', '30.jpg'];
    var bgArrayQuotes = ['1.png', '2.png', '3.png', '4.png'];
    var arrChecker = new Array();
    var path = 'assets/backImages/';
    var pathQuotes = 'assets/quotes/';

    var bg = bgArray[Math.floor(Math.random() * bgArray.length)];
    var bg2 = bgArrayQuotes[Math.floor(Math.random() * bgArrayQuotes.length)];
    var quoteImage = document.getElementById("main-quote-img");
    quoteImage.src = pathQuotes+bg2;

    arrChecker.push(bg);
    $('.image-main').css('background-image', 'url('+path+bg+')');

    setTimeout(function () {
        $('.map-wrapper').css('display', 'initial');
    }, 1000);


});
